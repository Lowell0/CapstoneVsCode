...
const OccupancyNoticetoComply = () => {
  ...
  const columns = [
    ...
    { field: "address", headerName: "Location", width: 100 },
...
const handleSearchInputChange = (e) => {
  ...
  const handleRefreshButtonClick = () => {
    fetchData();
    setSearchValue("");
    setFilteredData(complyApplication);
  };
  const filterData = () => {
    const filtered = complyApplication.filter((item) =>
  ...
  return (
    <Box m="15px">
      ...
      <Dialog open={isViewDialogOpen} onClose={handleCloseViewDialog}>
        <DialogTitle>Notice To Comply</DialogTitle>
        <DialogContent>
          {selectedItem && (
            <div>
